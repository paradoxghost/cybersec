...
def evaluate_model(bundle, X: pd.DataFrame, y: pd.Series, benign_label: str):
    ...
    return compute_classification_metrics(y.astype(str).to_numpy(), pred, list(bundle.label_encoder.classes_), benign_label, prob)
...
def main() -> None:
    ...
    comparison = pd.DataFrame([
        {"model": "baseline", "accuracy": baseline_m["accuracy"], "macro_f1": baseline_m["macro_f1"], "weighted_f1": baseline_m["weighted_f1"], "fpr_benign": baseline_m["false_positive_rate_on_benign"]},
        {"model": "stronger", "accuracy": stronger_m["accuracy"], "macro_f1": stronger_m["macro_f1"], "weighted_f1": stronger_m["weighted_f1"], "fpr_benign": stronger_m["false_positive_rate_on_benign"]},
    ])
    comparison.to_csv(f"{cfg['paths']['reports_metrics_dir']}/model_comparison.csv", index=False)

    save_confusion_matrix(baseline_m["confusion_matrix"], list(baseline.label_encoder.classes_), f"{cfg['paths']['reports_figures_dir']}/cm_baseline.png", "Baseline Confusion Matrix")
    save_confusion_matrix(stronger_m["confusion_matrix"], list(stronger.label_encoder.classes_), f"{cfg['paths']['reports_figures_dir']}/cm_stronger.png", "Stronger Model Confusion Matrix")
