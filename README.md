...
## Course Deliverables Support
- Docker build/run is supported via `Dockerfile` and `.dockerignore`.
- Final slides placeholder location: `deliverables/slides/`.
- One-page report draft location: `deliverables/report/one_page_report.md`.
- Video demo placeholder location: `deliverables/video_demo/`.

## Docker (Course Deliverable)
Docker Hub image name:
- `paradoxghost/cybersec-iot-ids:latest`

### Build locally
```bash
docker build -t paradoxghost/cybersec-iot-ids:latest .
```

### Run locally
```bash
docker run --rm -p 8000:8000 paradoxghost/cybersec-iot-ids:latest
```
...
```bash
docker pull paradoxghost/cybersec-iot-ids:latest
```
...
### Optional training step (if model artifacts are not yet present)
If `/health` returns `"model_loaded": false`, train artifacts first, then rerun container:
```bash
python scripts/run_training.py
```

### Data note
Raw CICIoT data is intentionally not bundled in the image. Provide the dataset at runtime by mounting a host folder and updating `configs/config.yaml` path fields as needed.
Example mount:
```bash
